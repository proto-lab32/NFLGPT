...
const MonteCarloSimulator = () => {
  ...
  // BASE weights; will be normalized by sum of abs
  const BASE = {
    EPA: 0.40, SR: 0.20, PPD_resid: 0.20, Xpl: 0.10, RZ: 0.10,
    ThreeOut_eff: -0.20, // must be NEGATIVE
    DVOA: 0.12, TO_EPA: 0.08, FP: 0.08, Penalty: 0.06,
  };
  const normalize = (obj) => {
    const s = Object.values(obj).reduce((a,b)=>a+Math.abs(b),0) || 1;
    const out = {}; for (const [k,v] of Object.entries(obj)) out[k]=v/s;
    return out;
  };
  ...
  const [numSimulations, setNumSimulations] = useState(30000);
  ...
  const parseCSV = async (file) => {
    ...
    while (i<text.length) {
      const ch = text[i];
      if (inQ) {
        if (ch === '"') { if (i+1<text.length && text[i+1]==='"'){field+='"';i+=2;} else {inQ=false;i++;} }
        else { field+=ch; i++; }
      } else {
        if (ch === '"') { inQ=true; i++; }
    ...
    const header = rows[0]; const data = rows.slice(1).map(r => {
      const o={}; for (let j=0;j<header.length;j++) o[header[j]]=(r[j]??"").trim(); return o;
    });
    return { header, data };
  };
  const pct = (v, def=null) => {
    if (v===null || v===undefined || v==="") return def;
    const s=String(v); const hasPct=s.includes("%"); const n=parseFloat(s.replace(/[% ,]/g,""));
    if (isNaN(n)) return def; return hasPct?n/100:n;
  };
  const num = (v, def=null) => {
    if (v===null || v===undefined || v==="") return def;
    const n=parseFloat(String(v).replace(/[, ]/g,"")); return isNaN(n)?def:n;
  };
  const aliases = {
    Team: ["Team","team","Name"],
    off_ppd:["Off PPD"], off_epa:["Off EPA/play"], off_sr:["Off Success Rate"],
    off_xpl:["Off Explosive Rate"], off_rz:["Off Red-Zone TD%"], off_3out:["Off 3-Out %"],
    def_ppd_allowed:["Def PPD Allowed"], def_epa_allowed:["Def EPA/play allowed"], def_sr:["Def Success Rate"],
    def_xpl:["Def Explosive Rate"], def_rz:["Def Red Zone TD %"], def_3out:["Def 3-Out %"],
    off_pen:["Off Penalties per Drive"], def_pen:["DEF Penalties per Drive"],
    off_to_epa:["Off TO EPA per Drive"], off_fp:["Off Avg Starting FP"],
    off_dvoa:["Off DVOA"], def_dvoa:["Def DVOA"],
    off_drives:["Off Drives/G"], def_drives:["Def Drives/G"],
    ed_pass:["Neutral Early-Down Pass %"], no_huddle:["No-Huddle %"],
  };
    ...
    try {
      ...
      data.forEach(row => {
        ...
        db[name] = {
          ...
          off_dvoa: pick(row, aliases.off_dvoa, 0),
          def_dvoa: pick(row, aliases.def_dvoa, 0),
  ...
  const computeNets = (team, opp) => {
    ...
    const out_eff = (z_def.OUT - z_off.OUT); // higher => tougher

    const ppd_resid = ppd_net - (C.c1*epa_net + C.c2*sr_net);

    const dvoa_net = (team.off_dvoa/100)/L.DVOA_sd + (opp.def_dvoa/100)/L.DVOA_sd; // SUM policy
    const z_to = (team.off_to_epa - 0)/L.TOEPA_sd;
    const z_fp = (team.off_fp - L.FP_mu)/L.FP_sd;
    const pen_mix = (-z_off.PEN + z_def.PEN);

    return { epa_net, sr_net, ppd_resid, xpl_net, rz_net, out_eff, dvoa_net, z_to, z_fp, pen_mix, z_off, z_def };
  };

  const ensembleZ = (n) => (
    W.EPA*n.epa_net + W.SR*n.sr_net + W.PPD_resid*n.ppd_resid + W.Xpl*n.xpl_net + W.RZ*n.rz_net +
    W.ThreeOut_eff*n.out_eff + W.DVOA*n.dvoa_net + W.TO_EPA*n.z_to + W.FP*n.z_fp + W.Penalty*n.pen_mix
  );
  ...
  const runOnce = (H, A, hfa) => {
    const nH = computeNets(H, A);
    const nA = computeNets(A, H);
    const zH = ensembleZ(nH); const zA = ensembleZ(nA);
    const ppdH = mapZtoPPD(zH) + (hfa/24); // minor per-drive HFA
    const ppdA = mapZtoPPD(zA);
    ...
    return { homeMean, awayMean, nH, nA, W, zH, zA, gameDrives, posShare };
  };
  ...
  const sanity = (nH, nA) => {
    return {
      combine_policy: "ADD offense_z + defense_allowed_z",
      dvoa_policy: "SUM off + def (def good is negative → reduces)",
      threeout_weight: "negative",
      weights_sum: Object.values(W).reduce((a,b)=>a+Math.abs(b),0).toFixed(3),
      W: Object.fromEntries(Object.entries(W).map(([k,v])=>[k, v.toFixed(3)])),
      home_terms: {
        epa_net: nH.epa_net.toFixed(3),
        sr_net: nH.sr_net.toFixed(3),
        ppd_resid: nH.ppd_resid.toFixed(3),
        dvoa_net: nH.dvoa_net.toFixed(3),
        out_eff: nH.out_eff.toFixed(3)
      },
      away_terms: {
        epa_net: nA.epa_net.toFixed(3),
        sr_net: nA.sr_net.toFixed(3),
        ppd_resid: nA.ppd_resid.toFixed(3),
        dvoa_net: nA.dvoa_net.toFixed(3),
        out_eff: nA.out_eff.toFixed(3)
      }
    };
  };
  ...
  const run = () => {
    ...
    const out = runOnce(H, A, hfaAdjustment);
    setSimulationResults({
      home: homeTeam, away: awayTeam,
      homeMean: out.homeMean, awayMean: out.awayMean,
      spread: out.homeMean - out.awayMean,
      gameDrives: out.gameDrives, posShare: out.posShare,
      zH: out.zH, zA: out.zA,
      sanity: sanity(out.nH, out.nA),
    });
  };

  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-4 flex items-center gap-2"><BarChart3 size={22}/> NFL Gaussian — v4 Sanity</h1>
        ...
        <div className="md:col-span-2 border rounded p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select className="border rounded px-3 py-2 w-full" value={homeTeam} onChange={(e)=>setHomeTeam(e.target.value)}>
            ...
            <div>
              ...
              <select className="border rounded px-3 py-2 w-full" value={awayTeam} onChange={(e)=>setAwayTeam(e.target.value)}>
                <option value="">Select</option>
                {teamList.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
            </div>
      ...
      {simulationResults && (
        <div className="border rounded p-4">
          <h2 className="font-semibold mb-3">Means (No Sampling)</h2>
          <div className="text-sm mb-2">Home mean: {simulationResults.homeMean.toFixed(2)} | Away mean: {simulationResults.awayMean.toFixed(2)} | Spread: {simulationResults.spread.toFixed(2)} | Drives: {simulationResults.gameDrives.toFixed(1)} | posShare: {(simulationResults.posShare*100).toFixed(1)}%</div>

          <div className="mt-3 border-t pt-3">
            <h3 className="font-semibold flex items-center gap-2"><ShieldAlert size={16}/> Sanity Panel</h3>
            <pre className="text-xs bg-gray-50 p-3 rounded overflow-auto">{JSON.stringify(simulationResults.sanity, null, 2)}</pre>
            <div className="text-xs text-gray-600 mt-2">
              Expect: <code>combine_policy = ADD</code>, <code>dvoa_policy = SUM</code>, <code>threeout_weight = negative</code>, <code>weights_sum ≈ 1.000</code>. If not, the wrong file is being imported.
            </div>
          </div>
        </div>
      )}
